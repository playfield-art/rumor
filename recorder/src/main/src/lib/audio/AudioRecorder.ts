...
export default class AudioRecorder extends EventEmitter {
  ...
  start () {
    ...
    this._childProcess = spawn(this._options.program, this._command.arguments, this._command.options);

    // Store this in `self` so it can be accessed in the callback.
    let self = this;
    this._childProcess.on('close', function (exitCode: number) {
      if (self._logger) {
        if(exitCode !== 0) self._logger.log(`AudioRecorder: Exit code '${exitCode}'.`);
      }
      self.emit('close', exitCode);
    });
    this._childProcess.on('error', function (error: any) {
